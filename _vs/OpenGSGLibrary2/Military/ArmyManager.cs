...
namespace Military
{
    public class ArmyManager
    {
        ...
        {
            if (provinceIdToArmiesTable_ == null) return new List<Army>();
            if (provinceIdToArmiesTable_.TryGetValue(provinceId, out var resultList) && resultList != null)
                return resultList;
        ...
        public void MoveArmy(Army movingArmy, int targetProvinceId)
        {
            ...
            if (!provinceIdToArmiesTable_.TryGetValue(targetProvinceId, out var newProvinceList) || newProvinceList == null)
            {
        ...
        private void UpdateProvinceToArmyTable()
        {
            ...
            foreach (var countryNationMil in nationMilitaryTable_)
            {
                foreach (var army in countryNationMil.Value.GetArmiesList())
                {
                    ...
                    if (!provinceIdToArmiesTable_.TryGetValue(location, out var armyList) || armyList == null)
                    {
